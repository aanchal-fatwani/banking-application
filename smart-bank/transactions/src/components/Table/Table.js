...
export default function CustomTable(props) {
  ...
  function stableSort(array, comparator) {
    ...
    const stabilizedThis = modified.map((el, index) => [el, index]);
    stabilizedThis.sort((a, b) => {
      const order = comparator(a[0], b[0]);
      if (order !== 0) return order;
      return a[1] - b[1];
    });
    let final = stabilizedThis.map((el) => el[0]);
    final = final.map((el) => {
      el.amount = org[el.index]["amount"];
      el.date = org[el.index]["date"];
      return el;
    });
    final = final.map((el) => {
      delete el.index;
                ...
                >
                  ...
                  {hasFeatureTab && (
                    <TableCell
                      ...
                      onClick={() =>
                        featureFn(
                          el.user_id ? el.user_id : el.item_id,
